...
class IntroViewController: AnimatedPagingScrollViewController {
    ...
    func createIntroViews() {
        ...
        searchLabel.font = Helper.defaultFontWith(.Regular, size: 7)
        ...
        let blendleLogo = UIImageView(image: UIImage(named: "BlendleLogo"))
        contentView.addSubview(blendleLogo)
        contentView.addConstraint(NSLayoutConstraint(item: blendleLogo, attribute: .CenterY, relatedBy: .Equal, toItem: contentView, attribute: .CenterY, multiplier: 1, constant: 0))
        keepView(blendleLogo, onPages: [6.4, 7], atTimes: [6, 7])
        
        alphaAnimation = AlphaAnimation(view: blendleLogo)
        alphaAnimation.addKeyframe(6.6, value: 0)
        alphaAnimation.addKeyframe(7, value: 1)
        animator.addAnimation(alphaAnimation)
        
        let blendleLabel = UILabel()
        blendleLabel.text = "Blendle"
        blendleLabel.textColor = UIColor.whiteColor()
        blendleLabel.font = Helper.defaultFontWith(.Semibold, size: 26)
        contentView.addSubview(blendleLabel)
        contentView.addConstraint(NSLayoutConstraint(item: blendleLabel, attribute: .Bottom, relatedBy: .Equal, toItem: blendleLogo, attribute: .Top, multiplier: 1, constant: -15))
        keepView(blendleLabel, onPages: [6.4, 7], atTimes: [6, 7])
        
        alphaAnimation = AlphaAnimation(view: blendleLabel)
        alphaAnimation.addKeyframe(6.6, value: 0)
        alphaAnimation.addKeyframe(7, value: 1)
        animator.addAnimation(alphaAnimation)
        ...
        againButton.titleLabel?.font = Helper.defaultFontWith(.Semibold, size: 18)
